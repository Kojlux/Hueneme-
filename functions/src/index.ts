...
const getAdminEmails = async (): Promise<string[]> => {
  ...
  return Array.from(
    new Set(
      [...baselineAdminEmails, ...snapshot.docs.map((document) => document.data().email)]
...
const formatContact = (data: NotificationData): string => {
...
const sendAdminNotification = async (
  kind: 'Order' | 'Custom request',
  id: string,
  data: NotificationData
): Promise<void> => {
  const recipients = await getAdminEmails();
  if (recipients.length === 0) {
    logger.warn('No admin email recipients found in admin_whitelist.', { kind, id });
    return;
  }

  const title = String(data.listingTitle || data.itemTitle || 'Untitled request');
  const requester = String(
    data.buyerDisplayName || data.requesterDisplayName || 'Unknown requester'
  );
...
export const notifyAdminsOfOrder = onDocumentCreated(
  {
  ...
  async (event) => {
    ...
    if (!data || !event.params.orderId) return;
    await sendAdminNotification('Order', event.params.orderId, data);
  }
);

export const notifyAdminsOfCustomRequest = onDocumentCreated(
  {
  ...
  async (event) => {
    const data = event.data?.data();
    if (!data || !event.params.requestId) return;
